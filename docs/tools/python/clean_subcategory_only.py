#!/usr/bin/env python3
"""
Clean only the subcategory field - keep all cases but set subcategory to empty string 
for entries that don't match approved organ systems.
"""
...
def clean_subcategory_field(input_file, output_file):
    """
    Keep all cases but clean only the subcategory field.
    Set subcategory to empty string if it doesn't match approved organ systems.
    """
    # Load the data
    ...
    # Process the data - keep all cases but clean subcategory
    ...
    print(f"Processed dataset: {len(data)} cases (all kept)")
    print(f"Cleaned subcategory field: {cleaned_count} cases set to empty string")
    
    # Print final organ system statistics
    print(f"\nValid organ systems ({len(mapping_stats)} total):")
    print("-" * 60)
    for organ_system, count in mapping_stats.most_common():
        print(f"{organ_system}: {count} cases")
    
    return len(data), cleaned_count

def main():
    """Main function to clean subcategory field only."""
    base_path = Path(__file__).parent.parent
    input_file = base_path / 'virtual-slides-filtered.json'
    output_file = base_path / 'virtual-slides-clean-subcategory.json'
    
    if not input_file.exists():
        print(f"Error: Input file not found: {input_file}")
        return
    
    print("Cleaning subcategory field only (keeping all cases)...")
    print("=" * 60)
    
    total_count, cleaned_count = clean_subcategory_field(input_file, output_file)
    
    print("=" * 60)
    print("Subcategory cleanup complete!")
    print(f"✅ Total cases: {total_count:,} (all kept)")
    print(f"🧹 Cleaned subcategories: {cleaned_count:,} set to empty string")
