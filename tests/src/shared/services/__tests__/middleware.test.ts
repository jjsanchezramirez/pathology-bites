...
import { createMockServerClient, mockUser, mockAdminUser } from '../../../__tests__/utils/supabase-mock'
...
describe('Middleware', () => {
  ...
  it('should allow admin users to access admin routes', async () => {
    ...
    mockClient.auth.getUser.mockResolvedValue({ data: { user: mockAdminUser }, error: null })
  ...
  it('should handle role check errors gracefully', async () => {
    ...
    expect(result).toEqual({ redirect: expect.any(Object) })
    expect(consoleSpy).toHaveBeenCalledWith('Error checking admin/reviewer role:', expect.any(Error))
  ...
  it('should check admin role from user metadata first', async () => {
    ...
    expect(result).toBe(mockResponse)
    expect(consoleSpy).toHaveBeenCalledWith('Admin/Reviewer access granted via metadata')
  ...
  it('should preserve redirect parameter in login URL', async () => {
    ...
    expect(mockClonedUrl.searchParams.set).toHaveBeenCalledWith('redirect', '/protected-page')
  })
})
