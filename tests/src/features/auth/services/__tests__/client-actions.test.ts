...
import { createMockSupabaseClient, mockSession } from '../../../../__tests__/utils/supabase-mock'
import { setupNavigationMocks, resetAllMocks } from '../../../../__tests__/utils/auth-test-utils'
...
describe('useAuthActions', () => {
  ...
  describe('refreshSession', () => {
    ...
    it('should handle refresh session errors', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Session refresh error:', refreshError)
    ...
    it('should handle refresh session network errors', async () => {
      ...
      await expect(result.current.refreshSession()).rejects.toThrow('Network error')

      expect(consoleSpy).toHaveBeenCalledWith('Error refreshing session:', networkError)
  ...
  describe('getSession', () => {
    ...
    it('should handle get session errors', async () => {
      ...
      await expect(result.current.getSession()).rejects.toThrow('Session error')

      expect(consoleSpy).toHaveBeenCalledWith('Get session error:', sessionError)
    ...
    it('should handle get session network errors', async () => {
      ...
      await expect(result.current.getSession()).rejects.toThrow('Network error')

      expect(consoleSpy).toHaveBeenCalledWith('Error getting session:', networkError)
