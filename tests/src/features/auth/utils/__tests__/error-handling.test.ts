...
import {
  ...
  RetryManager,
  AuthErrorType,
} from "@/features/auth/utils/error-handling";
...
describe("Utility functions", () => {
  it("should identify retryable errors", () => {
    ...
    expect(isRetryableError(new Error("Invalid login credentials"))).toBe(
      false,
    );
  });
  ...
  it("should get error severity", () => {
    ...
    expect(getErrorSeverity(new Error("Invalid login credentials"))).toBe(
      "medium",
    );
