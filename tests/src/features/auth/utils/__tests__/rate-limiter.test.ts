...
describe('Rate Limiter', () => {
  ...
  describe('createRateLimitResponse', () => {
    beforeEach(() => {
      ...
      global.Response = jest.fn().mockImplementation((body, options) => ({
        ...
        headers: {
          get: jest.fn().mockImplementation((name) => {
            if (name === 'Content-Type') return options?.headers?.['Content-Type']
            if (name === 'Retry-After') return options?.headers?.['Retry-After']
            return null
          })
        },
  ...
  describe('Rate limiter edge cases', () => {
    ...
    it('should handle concurrent requests safely', () => {
      ...
      const results = []
      for (let i = 0; i < 10; i++) {
        results.push(loginRateLimiter.checkLimit('concurrent@example.com', 'login'))
      }
