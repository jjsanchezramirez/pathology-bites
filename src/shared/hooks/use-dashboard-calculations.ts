...
export function useDashboardCalculations({ recentQuizzes, stats }: UseDashboardCalculationsProps) {
  const weeklyProgress = useMemo(() => {
    ...
    const weeklyQuizzes = recentQuizzes.filter((quiz) => new Date(quiz.completedAt) >= weekStart);
  ...
  const monthlyProgress = useMemo(() => {
    ...
    const monthlyQuizzes = recentQuizzes.filter((quiz) => new Date(quiz.completedAt) >= monthStart);

    const avgMonthlyScore =
      monthlyQuizzes.length > 0
        ? Math.round(
            monthlyQuizzes.reduce((sum, quiz) => sum + quiz.score, 0) / monthlyQuizzes.length
          )
        : 0;
