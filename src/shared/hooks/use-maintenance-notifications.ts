...
export function useMaintenanceNotifications(options: MaintenanceNotificationOptions = {}) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (data?.code === "42501" && data?.message?.includes("row-level security policy")) {
          throw new Error(
            "You don't have permission to subscribe for notifications. Please contact the administrator."
          );
        }

        // Check for duplicate email
        if (data?.code === "23505" && data?.message?.includes("duplicate key")) {
          // This is actually a "success" case - email is already subscribed
          options.onSuccess?.();
          return;
        }
        ...
        throw new Error(
          data?.message || data?.error || "Failed to subscribe. Please try again later."
        );
      }
