// Canonical lesson model — the single source of truth for interactive sequences.
// All spatial coordinates are in 0–100 percentage space on a 16:9 canvas. The
// camera transform space (−50..50) is an internal detail of the evaluator.
//
// (Moved here from app/(admin)/admin/lesson-studio/model/types.ts so both the
// admin editor and the public player can import it without crossing route folders.)
...
/**
 * Unified camera operation.
 * persistent=false (default): zoom — returns to prior base after hold+fadeOut.
 * persistent=true: pan — target becomes the new base once fadeIn completes.
 */
export interface CameraElement extends ElementBase {
  kind: "camera";
  /** Target camera state in 0–100 percent UI coords + scale. */
  to: { x: number; y: number; scale: number };
  persistent: boolean;
  /** Easing curve for the move into `to`. Defaults to easeInOutCubic. */
  easing?: EaseName;
}
...
export interface LessonAudio {
  ...
  transcript?: string;
  duration?: number;
  /** Forced-alignment word timestamps (absolute seconds), when available. */
  wordTimings?: WordTiming[];
}

/** A single aligned word with absolute start/end (seconds from audio start). */
...
export interface Lesson {
  /** Bumped when the persisted shape changes; absent in pre-collapse rows. */
