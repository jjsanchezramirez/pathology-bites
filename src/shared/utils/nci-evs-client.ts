...
async function loadPrecomputedExpansions(): Promise<void> {
  ...
  precomputedExpansionsPromise = (async () => {
    try {
      ...
      console.log(`[NCI EVS] ✓ Loaded ${Object.keys(precomputedExpansions!).length} pre-computed expansions`)

    } catch (error) {
      console.warn('[NCI EVS] Failed to load pre-computed expansions, will use API fallback:', error)
...
async function searchNCIEVSClient(term: string): Promise<string[]> {
  try {
    ...
    const response = await fetch(`/api/public/tools/nci-evs-expand?term=${encodeURIComponent(term)}`, {
      signal: controller.signal,
      headers: {
...
export async function expandSearchTermClient(term: string): Promise<string[]> {
  ...
  if (precomputedExpansions && precomputedExpansions[normalized]) {
    ...
    console.log(`[Expansion Client] Using pre-computed expansions for "${term}" (${expansions.length} terms)`)
