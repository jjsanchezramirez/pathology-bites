...
export async function extractBookMetadata(isbn: string): Promise<CitationData> {
  try {
    ...
    const response = await fetch(`/api/extract-book-metadata?isbn=${encodeURIComponent(cleanIsbn)}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
...
export async function extractJournalMetadata(doi: string): Promise<CitationData> {
  try {
    ...
    const response = await fetch(`/api/extract-journal-metadata?doi=${encodeURIComponent(cleanDoi)}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
