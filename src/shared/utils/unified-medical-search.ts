...
const ORGAN_SYSTEM_MAPPINGS: { [key: string]: string[] } = {
  ...
  breast: ["mammary", "breast", "ductal", "lobular", "nipple", "areolar"],
  dermatopathology: [
    "cutaneous",
    "skin",
    "dermal",
  ...
  neuropathology: ["neural", "brain", "cerebral", "meningeal", "spinal", "neuronal", "glial"],
  bone: ["osseous", "bone", "osteoid", "chondroid", "cartilaginous", "skeletal"],
  "soft-tissue": ["muscular", "adipose", "fibrous", "vascular", "synovial", "connective tissue"],
  gynecological: [
    "endometrial",
    "cervical",
    "ovarian",
...
function generateWordOrderVariations(text: string): string[] {
  ...
  if (words.length === 3) {
    ...
    if (
      descriptors.includes(word1.toLowerCase()) &&
  ...
  if (words.length === 2) {
    ...
    if (descriptors.includes(word1.toLowerCase()) && tumorTypes.includes(word2.toLowerCase())) {
      ...
      for (const organ of organContexts) {
        ...
        console.log(
          `[Word Order] Context expansion: "${word1} ${word2}" → "${organ} ${word1} ${word2}" and "${word1} ${organ} ${word2}"`
        );
      }
    }
  }
...
function isRelevantMedicalWord(word: string): boolean {
  ...
  return (
    ...
    /(?:oma|itis|osis|pathy|trophy|plasia|carci|lymph|sarco)$/.test(word)
  ); // Medical suffixes
}
...
export function calculateUnifiedMatchScore(
  ...
  for (const phrase of medicalTerms.exactPhrases) {
    ...
    // Use both normalized and original to catch both "b-cell" and "b cell" variants
      ...
      (topicNormalized.includes(phraseNormalized) || topicLower.includes(phrase)) &&
      phrase.length >= 6
    ) {
      // Check for word boundary matches vs substring matches (use normalized version)
      const isWordBoundaryMatch = new RegExp(
        `\\b${phraseNormalized.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`
      ).test(topicNormalized);
    ...
    // Exact phrase in lesson name (use normalized comparison)
      ...
      (lessonNormalized.includes(phraseNormalized) || lessonLower.includes(phrase)) &&
      phrase.length >= 6
    ) {
      const isWordBoundaryMatch = new RegExp(
        `\\b${phraseNormalized.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`
      ).test(lessonNormalized);
  ...
  for (const synonym of medicalTerms.synonyms) {
    ...
    const synonymRegex = new RegExp(
      `\\b${synonym.toLowerCase().replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`
    );
  ...
  for (const phrase of medicalTerms.exactPhrases) {
    if (contentLower.includes(phrase)) {
      const matches = (
        contentLower.match(new RegExp(phrase.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g")) || []
      ).length;
  ...
  for (const synonym of medicalTerms.synonyms) {
    if (contentLower.includes(synonym.toLowerCase())) {
      const matches = (
        contentLower.match(
          new RegExp(synonym.toLowerCase().replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g")
        ) || []
      ).length;
  ...
  if (category) {
    ...
    if (
      ...
      lessonLower.includes(categoryLower) ||
      ...
      if (options.logDetailedScoring) {
        console.log(
          `🏷️  CATEGORY MATCH: "${category}" → +${categoryBonus}, ${categoryMultiplier}x multiplier`
        );
      }
    }
  }
  ...
  if (subcategory) {
    ...
    if (
      topicLower.includes(subcategoryLower) ||
      lessonLower.includes(subcategoryLower) ||
      contentLower.includes(subcategoryLower)
    ) {
      categoryBonus += 15000;
      ...
      if (options.logDetailedScoring) {
        console.log(
          `🏷️  SUBCATEGORY MATCH: "${subcategory}" → +${categoryBonus}, ${subcategoryMultiplier}x multiplier`
        );
      }
    }
  }
