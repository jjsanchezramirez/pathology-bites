...
} from "@/shared/config/category-color-map";
...
export const getCategoryColor = (category: CategoryColorData): string => {
  ...
  const colorPalette = isAPSubspecialty
    ? strongColors.slice(1) // Skip first color (reserved for AP)
    : isCPSubspecialty
      ? lightColors.slice(1) // Skip first color (reserved for CP)
