...
async function loadVirtualSlidesData(): Promise<VirtualSlide[]> {
  ...
  try {
    ...
    const uniqueSlides = entries.filter(
      (slide) =>
        slide.image_url &&
    ...
    console.log(
      `[Client WSI] Loaded ${uniqueSlides.length} slides directly from R2 in ${loadTime}ms`
    );
...
export async function selectRandomWSIClientSide(
  ...
  try {
    ...
    if (categoryFilter && categoryFilter !== "all") {
      ...
      console.log(
        `[Client WSI] After category filter '${categoryFilter}': ${slides.length} slides`
      );
    }
