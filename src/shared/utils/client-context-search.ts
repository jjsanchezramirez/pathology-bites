...
async function loadContentFile(filename: string): Promise<unknown> {
  ...
  try {
    ...
    console.log(
      `[Client Context] Loaded ${filename} (${Math.round(JSON.stringify(data).length / 1024)}KB) in ${loadTime}ms`
    );
...
function getTargetedFiles(category: string, subcategory: string, _diagnosis: string): string[] {
  ...
  if (
    subcategoryLower.includes("hematopathology") ||
    subcategoryLower.includes("blood") ||
    subcategoryLower.includes("lymph")
  ) {
    ...
    subcategoryLower.includes("gynecolog") ||
    subcategoryLower.includes("cervix") ||
    subcategoryLower.includes("ovary")
  ) {
    primaryFiles.push("ap-gynecological.json");
  } else if (
    subcategoryLower.includes("genitourinary") ||
    subcategoryLower.includes("kidney") ||
    subcategoryLower.includes("prostate")
  ) {
    primaryFiles.push("ap-genitourinary.json");
  } else if (
    subcategoryLower.includes("gastrointestinal") ||
    subcategoryLower.includes("stomach") ||
    subcategoryLower.includes("colon")
  ) {
    primaryFiles.push("ap-gastrointestinal.json");
  } else if (
    subcategoryLower.includes("head") ||
    subcategoryLower.includes("neck") ||
    subcategoryLower.includes("thyroid")
  ) {
    primaryFiles.push("ap-head-and-neck---endocrine.json");
  } else if (
    subcategoryLower.includes("liver") ||
    subcategoryLower.includes("pancrea") ||
    subcategoryLower.includes("biliary")
  ) {
...
function calculatePhraseMatching(
  ...
  if (diagnosticWords.length >= 2) {
    ...
    for (let i = 0; i < diagnosticWords.length - 1; i++) {
      ...
      const wordPairRegex = new RegExp(
        `\\b${wordPair.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`,
        "i"
      );
...
function calculateFullTextScore(
  ...
  const cleanTerms = allTerms.filter(
    (term) => term.length > 3 && !isGenericTerm(term.toLowerCase())
  );
...
export async function findContextClientSide(
  ...
  try {
    ...
    console.log(
      `[Client Context] Category: ${category || "Not specified"}, Subcategory: ${subcategory || "Not specified"}`
    );
    ...
    console.log(
      `[Client Context] Will search ${targetFiles.length} files: ${targetFiles.join(", ")}`
    );
          ...
          if (lessonData && typeof lessonData === "object" && "topics" in lessonData) {
            ...
            for (const [topicName, topicData] of Object.entries(topics)) {
              if (topicData && typeof topicData === "object" && "content" in topicData) {
                ...
                let score = calculateFullTextScore(
                  diagnosticTerms,
                  topicNameLower,
                  lessonNameLower,
                  fullContentText
                ...
                if (category) {
                  const categoryLower = category.toLowerCase();
                  if (
                    topicNameLower.includes(categoryLower) ||
                    ...
                    console.log(
                      `[Client Context] STRONG category match: "${categoryLower}" -> +${categoryBonus} and 2x multiplier`
                    );
                  }
                }
                ...
                if (subcategory) {
                  ...
                  if (
                    ...
                    console.log(
                      `[Client Context] STRONG subcategory match: "${subcategoryLower}" -> +${categoryBonus} and 3x multiplier`
                    );
                  }
                }
                ...
                const specificityPenalty = validateDiagnosticSpecificity(
                  diagnosis,
                  topicName,
                  lessonName,
                  fullContentText
                ...
                if (specificityPenalty > 0) {
                  ...
                  console.log(
                    `[Client Context] Diagnostic specificity penalty: -${specificityPenalty}`
                  );
                }
                ...
                if (score > 0) {
                  ...
                  if (!bestMatch || score > bestMatch.score) {
                    ...
                    console.log(
                      `[Client Context] New best match: ${filename} - ${lessonName} - ${topicName} (score: ${score})`
                    );

                    // Early termination for excellent matches (from server optimization)
                    if (score >= 8000) {
                      console.log(
                        `[Client Context] Excellent match found (score: ${score}) - terminating search early`
                      );
                      break;
                    }
                  }
    ...
    if (!bestMatch) {
      ...
      console.log(
        `[Client Context] Very poor context match (score: ${bestMatch.score}) - rejecting slide`
      );
      shouldReject = true;
      contextQuality = "poor";
    } else if (bestMatch.score < 8000) {
      console.log(
        `[Client Context] Moderate context match (score: ${bestMatch.score}) - proceeding with caution`
      );
      shouldReject = false;
      ...
      console.log(
        `[Client Context] Excellent context match (score: ${bestMatch.score}) - high confidence match`
      );
 ...
 */
function validateDiagnosticSpecificity(
  diagnosis: string,
  topicName: string,
  lessonName: string,
  fullContentText: string
  ...
  for (const [specificDiagnosis, requiredTerms] of Object.entries(specificTerms)) {
    if (diagnosisLower.includes(specificDiagnosis.toLowerCase())) {
      ...
      const termsFound = requiredTerms.filter(
        (term) =>
          combinedContent.includes(term.toLowerCase()) ||
          fullContentText.includes(term.toLowerCase())
      ).length;
      ...
      if (termsFound < requiredTerms.length * requiredPercentage) {
        penalty += 3000;
        console.log(
          `[Client Context] Specific diagnosis "${specificDiagnosis}" missing key terms: ${termsFound}/${requiredTerms.length}`
        );
      }
    }
  }
  ...
  if (diagnosisLower.includes("carcinoma")) {
    ...
    if (diagnosisSubtype) {
      ...
      const contentHasSubtype =
        combinedContent.includes(diagnosisSubtype) || fullContentText.includes(diagnosisSubtype);
      const contentIsGeneric =
        combinedContent.includes("carcinoma") &&
        !carcinomaSubtypes.some((st) => st !== diagnosisSubtype && combinedContent.includes(st));

      if (!contentHasSubtype && !contentIsGeneric) {
        penalty += 2000;
        console.log(
          `[Client Context] Carcinoma subtype mismatch: diagnosis has "${diagnosisSubtype}" but content doesn't`
        );
      }
    }
  }
  ...
  if (diagnosisLower.includes("lymphoma")) {
    ...
    const diagnosisSubtype = lymphomaSubtypes.find((subtype) =>
      diagnosisLower.includes(subtype.replace("/", ""))
    );

    if (diagnosisSubtype) {
      const contentHasSubtype =
        combinedContent.includes(diagnosisSubtype.replace("/", "")) ||
        fullContentText.includes(diagnosisSubtype.replace("/", ""));

      if (!contentHasSubtype) {
        penalty += 2500;
        console.log(
          `[Client Context] Lymphoma subtype mismatch: diagnosis has "${diagnosisSubtype}" but content doesn't`
        );
      }
    }
  }
    ...
    if (diagnosisLower.includes(organTerm)) {
      for (const conflictTerm of conflictingTerms) {
        if (combinedContent.includes(conflictTerm) || fullContentText.includes(conflictTerm)) {
          penalty += 1500;
          console.log(
            `[Client Context] Organ system mismatch: diagnosis has "${organTerm}" but content mentions "${conflictTerm}"`
          );
