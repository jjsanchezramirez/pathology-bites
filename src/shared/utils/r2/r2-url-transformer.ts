...
const getR2PublicUrl = () => {
  return (
    process.env.CLOUDFLARE_R2_PUBLIC_URL || "https://pub-a4bec7073d99465f99043c842be6318c.r2.dev"
  );
};
...
export function transformAnkomaData(ankomaData: unknown): unknown {
  ...
  const extractAndTransformImagePaths = (
    htmlContent: string
  ): { html: string; imagePaths: string[] } => {
