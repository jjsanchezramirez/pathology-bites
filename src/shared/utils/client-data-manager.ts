...
class ClientDataManager {
    ...
    return {
      ...
      totalProgress: {
        ...
        percentage: Math.round(
          (((wsiProgress.loaded ? 1 : 0) + contentProgress.loaded) / (1 + contentProgress.total)) *
            100
        ),
      },
    };
  ...
  private async _loadWSIMetadata(): Promise<VirtualSlide[]> {
    ...
    try {
      ...
      console.log(
        `[DataManager] ✅ WSI metadata loaded directly from R2: ${data.length} slides, ${(sizeBytes / (1024 * 1024)).toFixed(1)}MB in ${loadTime}ms`
      );
      return data;
    } catch (error) {
  ...
  async searchWSIs(criteria: {
    ...
    if (criteria.category) {
      ...
      filtered = filtered.filter((slide) => slide.category.toLowerCase().includes(categoryLower));
    }
    ...
    if (criteria.diagnosis) {
      ...
      filtered = filtered.filter((slide) => slide.diagnosis.toLowerCase().includes(diagnosisLower));
    }

    if (criteria.repository) {
      filtered = filtered.filter((slide) => slide.repository === criteria.repository);
    }
