// Hardcoded category color mappings
// This file contains the static color assignments for all categories
// to avoid database queries for color information
...
// Hardcoded category data with colors
export const CATEGORY_COLOR_MAP: CategoryColorMapping[] = [
  {
...
export const CATEGORY_BY_ID = new Map(CATEGORY_COLOR_MAP.map((cat) => [cat.id, cat]));

export const CATEGORY_BY_SHORT_FORM = new Map(
  CATEGORY_COLOR_MAP.map((cat) => [cat.shortForm, cat])
);

export const CATEGORY_BY_NAME = new Map(CATEGORY_COLOR_MAP.map((cat) => [cat.name, cat]));

// Helper functions
export function getCategoryColorById(id: string): string | undefined {
  return CATEGORY_BY_ID.get(id)?.color;
}

export function getCategoryColorByShortForm(shortForm: string): string | undefined {
  return CATEGORY_BY_SHORT_FORM.get(shortForm)?.color;
}

export function getCategoryColorByName(name: string): string | undefined {
  return CATEGORY_BY_NAME.get(name)?.color;
}

export function getCategoryById(id: string): CategoryColorMapping | undefined {
  return CATEGORY_BY_ID.get(id);
}

export function getCategoryByShortForm(shortForm: string): CategoryColorMapping | undefined {
  return CATEGORY_BY_SHORT_FORM.get(shortForm);
}

export function getCategoryByName(name: string): CategoryColorMapping | undefined {
  return CATEGORY_BY_NAME.get(name);
}

// Get parent category
export function getParentCategory(categoryId: string): CategoryColorMapping | undefined {
  const category = CATEGORY_BY_ID.get(categoryId);
  if (!category?.parentId) return undefined;
  return CATEGORY_BY_ID.get(category.parentId);
}

// Get all subcategories of a parent
export function getSubcategories(parentId: string): CategoryColorMapping[] {
  return CATEGORY_COLOR_MAP.filter((cat) => cat.parentId === parentId);
}

// Get all top-level categories
export function getTopLevelCategories(): CategoryColorMapping[] {
  return CATEGORY_COLOR_MAP.filter((cat) => cat.level === 1);
}
