...
export function DiagnosticContent({ content, entity }: DiagnosticContentProps) {
  ...
  if (isAIParsed) {
    ...
    if (
      hasContent(content["DIFFERENTIAL DIAGNOSIS"]) ||
      hasContent(content.DIFFERENTIAL_DIAGNOSIS)
    ) {
      sections.push({
        title: "Differential Diagnosis",
        icon: Microscope,
  ...
  const addSmartSpacing = (text: string): string => {
    return (
      text
        ...
        .replace(
          /(positive|negative|stain|marker|mutation|translocation|expression|antibody|cells)/gi,
          " $1"
        )
