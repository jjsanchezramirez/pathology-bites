// src/shared/components/common/session-debug.tsx
'use client'
...
export function SessionDebugComponent() {
  ...
  useEffect(() => {
    ...
    // Set up auth listener using shared service
    console.log('👂 Setting up shared auth state listener...')
    const unsubscribe = realtimeService.addAuthListener((event, session) => {
      console.log('🔄 Shared auth state change:', { event, hasSession: !!session, userEmail: session?.user?.email })
      setDebugInfo(prev => ({
        ...prev,
