...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!quizSession) {
      log.error(`[Quiz Results] Session not found: ${id}`);
    ...
    // Get quiz results
    log.debug(`[Quiz Results] Fetching results for session: ${id}`);
    const results = await quizService.getQuizResults(id, supabase);

    if (!results) {
      log.error(`[Quiz Results] Results not available for session: ${id}`);
      return NextResponse.json(
    ...
    log.debug(`[Quiz Results] Results fetched successfully for session: ${id}`);

    // Get recently unlocked achievements that haven't been shown yet (non-blocking)
    let newAchievements = [];
    try {
      log.debug("[Quiz Results] Fetching recent unshown achievements for user:", userId);
      ...
      if (newAchievements.length > 0) {
        ...
        log.debug("[Quiz Results] No recent unshown achievements found");
      }
    } catch (achievementError) {
      // Don't fail results fetch if achievement check fails
      log.error("[Quiz Results] Failed to fetch achievements:", achievementError);
    }
    ...
    log.error("Error fetching quiz results:", error);
