...
export async function GET(request: NextRequest) {
  try {
    ...
    const [statsResult, activitiesResult, settingsResult, recentQuizzesResult] = await Promise.all([
  ...
  try {
    ...
    const [sessionsResult, attemptsResult] = await Promise.all([
      ...
      supabase
        ...
        .in("quiz_session_id", supabase.from("quiz_sessions").select("id").eq("user_id", userId)),
    ]);
...
function calculateStudyStreak(completedSessions: unknown[]): number {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (sessionsByDate.has(dateString)) {
      ...
      // Don't break on first day if no activity
      break;
    }
  }
