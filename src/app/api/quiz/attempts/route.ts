...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (session.user_id !== userId) {
      console.error("[API] /api/quiz/attempts - Unauthorized access attempt:", {
    ...
    if (existingAttemptError) {
      console.error(
        "[API] /api/quiz/attempts - Error checking for existing attempt:",
        existingAttemptError
      );
    } else if (existingAttempt) {
      console.log(
        "[API] /api/quiz/attempts - Duplicate submission detected, returning existing attempt"
      );
      ...
      if (fullAttemptError) {
        console.error(
          "[API] /api/quiz/attempts - Error fetching existing attempt:",
          fullAttemptError
        );
      } else {
        return NextResponse.json({
    ...
    if (selectedAnswerId) {
      ...
      if (answerError || !answerOption) {
        console.error("[API] /api/quiz/attempts - Selected answer not found:", {
          selectedAnswerId,
          answerError,
        });
        return NextResponse.json({ error: "Selected answer not found" }, { status: 400 });
      }
    ...
    if (firstAnswerId) {
      ...
      if (firstAnswerError || !firstAnswerOption) {
        console.error("[API] /api/quiz/attempts - First answer not found:", {
          firstAnswerId,
          firstAnswerError,
        });
        return NextResponse.json({ error: "First answer not found" }, { status: 400 });
      }
    ...
    console.log(
      "[API] /api/quiz/attempts - Submitting quiz attempt with database triggers enabled"
    );
