...
export async function POST(request: NextRequest) {
  try {
    ...
    if (session.status === "completed") {
      return NextResponse.json({ error: "Quiz session is already completed" }, { status: 400 });
    }
    ...
    console.log(
      `[Batch Answers] Successfully submitted ${newAttemptData.length} answers for session ${sessionId}`
    );
