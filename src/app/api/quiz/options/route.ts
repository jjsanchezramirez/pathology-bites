...
async function getFallbackUserStats(
  ...
  try {
    ...
    const categoryQuestions = new Map<
      string,
      Map<string, { hasCorrect: boolean; hasIncorrect: boolean }>
    >();
    ...
    for (const attempt of attempts) {
      ...
      if (categoryQuestionMap) {
        const existing = categoryQuestionMap.get(questionId) || {
          hasCorrect: false,
          hasIncorrect: false,
        };
...
export async function GET() {
  ...
  try {
    ...
    if (cached && Date.now() - cached.timestamp < CACHE_TTL && cached.version === userVersion) {
    ...
    console.log(
      `Found ${subcategories.length} categories with ${questionCounts.length} total questions`
    );
    ...
    const categoriesWithStats = subcategories.map((category) => {
      ...
      console.log(
        `Category ${category.name}: ${questionCount} questions, Parent: ${parentName} (${parent}), User stats:`,
        stats
      );
