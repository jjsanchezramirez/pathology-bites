...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!answerOptions || !Array.isArray(answerOptions)) {
      return NextResponse.json({ error: "Invalid answer options data" }, { status: 400 });
    }
    ...
    const { data, error } = await supabase.from("question_options").insert(answerOptions).select();
