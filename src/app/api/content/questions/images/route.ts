...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!questionImages || !Array.isArray(questionImages)) {
      return NextResponse.json({ error: "Invalid question images data" }, { status: 400 });
    }
    ...
    const { data, error } = await supabase.from("question_images").insert(questionImages).select();
