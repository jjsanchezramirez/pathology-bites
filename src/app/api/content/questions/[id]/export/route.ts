...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const exportData = {
      ...
      answer_options:
        question.question_options?.map((option: unknown) => ({
          text: option.text,
      ...
      question_images:
        question.question_images?.map((qi: unknown) => ({
      ...
      metadata: {
        ...
        question_set:
          question.question_set && !Array.isArray(question.question_set)
            ? {
        ...
        category:
          question.category && !Array.isArray(question.category)
            ...
            : null,
        tags:
          question.question_tags?.map((qt: unknown) => ({
            id: qt.tag.id,
            name: qt.tag.name,
          })) || [],
