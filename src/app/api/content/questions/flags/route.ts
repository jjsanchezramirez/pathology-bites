  ...
  try {
    ...
    if (question.status !== "published") {
      return NextResponse.json(
        { error: "Only published questions can be flagged" },
        { status: 400 }
      );
    }
    ...
    if (flagCount >= FLAG_THRESHOLD) {
      ...
      if (currentQuestion && currentQuestion.status === "published") {
        ...
        const { error: reviewError } = await supabase.from("question_reviews").insert({
          ...
          action: "flagged",
          feedback:
            `Question flagged by user (${flagCount} flag${flagCount > 1 ? "s" : ""}): ${flag_type}. ${description || ""}`.trim(),
        });
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (userError || !["admin", "reviewer"].includes(userData?.role)) {
      return NextResponse.json(
        { error: "Forbidden - Admin or Reviewer access required" },
        { status: 403 }
      );
    }
    ...
    if (status === "closed") {
      ...
      updateData.resolution_type =
        resolution_notes && resolution_notes.trim() ? "fixed" : "dismissed";
    }
