...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error } = await supabase.from("demo_questions").select("*").eq("id", id).single();
    ...
    if (error) {
      ...
      try {
        ...
        const explanationImage: QuestionImage | null =
          explanationImages.length > 0 ? explanationImages[0] : null;
