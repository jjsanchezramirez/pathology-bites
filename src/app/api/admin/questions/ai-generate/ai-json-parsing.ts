...
export function extractJSON(text: string): unknown {
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            const errorMsg = `AI model returned invalid JSON. Try: (1) Use a different model like GPT-OSS 120B (Groq) or Gemini 3.5 Flash-Lite, (2) Simplify instructions, or (3) Try again (responses vary). Error: ${finalError instanceof Error ? finalError.message : "Parse error"}`;
  ...
  throw new Error(
    "No JSON found in AI response. The model may have returned plain text instead of JSON. Try using a different AI model (e.g., GPT-OSS 120B on Groq or Gemini 3.5 Flash-Lite) or simplify your instructions."
  );
}
