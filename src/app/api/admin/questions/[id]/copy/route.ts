...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const canCopy =
      originalQuestion.created_by === user.id || originalQuestion.status === "published";
    ...
    if (createError || !newQuestion) {
      ...
      return NextResponse.json({ error: "Failed to create question copy" }, { status: 500 });
    }
    ...
    if (originalQuestion.question_options && originalQuestion.question_options.length > 0) {
      ...
      const { error: optionsError } = await supabase.from("question_options").insert(newOptions);
    ...
    if (originalQuestion.question_images && originalQuestion.question_images.length > 0) {
      ...
      const { error: imagesError } = await supabase.from("question_images").insert(newImages);
    ...
    if (originalQuestion.question_tags && originalQuestion.question_tags.length > 0) {
      ...
      const { error: tagsError } = await supabase.from("question_tags").insert(newTags);
