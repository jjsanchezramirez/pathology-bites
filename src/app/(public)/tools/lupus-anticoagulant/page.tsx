...
export default function LupusAnticoagulantPage() {
  ...
  const validateForm = (): boolean => {
    ...
    if (
      values.hex_la_delta === undefined ||
      isNaN(values.hex_la_delta) ||
      values.hex_la_delta < 0
    ) {
  ...
  const generateLupusAnticoagulantReport = (values: LACValues): LACReport => {
    ...
    if (values.inr > 3.5) {
      report.caveats.push(
        `High INR (${values.inr.toFixed(2)}) significantly interferes with phospholipid-dependent clotting assays. Interpretation for LAC is unreliable. This may be due to warfarin or DOACs.`
      );
    ...
    if (values.heparin_xa >= ref.heparin_xa.critical) {
      report.caveats.push(
        `High heparin anti-Xa level (${values.heparin_xa.toFixed(2)} IU/mL) significantly interferes with clotting assays. Interpretation for LAC is unreliable.`
      );
      report.overall = "Uninterpretable";
      return report;
    }
    ...
    if (values.tt > 50) {
      report.caveats.push(
        `Markedly prolonged Thrombin Time (${values.tt.toFixed(1)}s) suggests presence of a direct thrombin inhibitor (e.g., Dabigatran) or high-dose heparin, making LAC assessment unreliable.`
      );
      report.overall = "Uninterpretable";
      return report;
    }
    ...
    if (values.inr > ref.inr.high) {
      report.caveats.push(
        `INR is elevated (${values.inr.toFixed(2)}), which can be caused by anticoagulants (Warfarin, DOACs), liver disease, or factor deficiencies. This may influence results.`
      );
    }

    if (values.heparin_xa > 0.1) {
      report.caveats.push(
        `Heparin effect is present (Anti-Xa: ${values.heparin_xa.toFixed(2)} IU/mL). While assays are designed to mitigate this, a strong effect can still influence results.`
      );
    }

    if (values.tt > ref.tt.high && values.heparin_xa < 0.1) {
      report.caveats.push(
        `Thrombin Time is prolonged (${values.tt.toFixed(1)}s) in the absence of significant heparin, suggesting potential direct thrombin inhibitor, dysfibrinogenemia, or DIC.`
      );
    }
    ...
    if (values.ptt_la > ref.ptt_la.high) {
      if (values.hex_la_delta > ref.hex_la_delta.positive) {
        report.findings.ptt =
          "aPTT is prolonged and demonstrates correction in the hexagonal phase assay. This finding is POSITIVE for a lupus anticoagulant.";
        isLACPositive = true;
      } else {
        report.findings.ptt =
          "aPTT is prolonged but does not correct in the hexagonal phase assay. This is more suggestive of a factor deficiency (intrinsic pathway), a specific factor inhibitor, or an anticoagulant effect rather than a typical lupus anticoagulant.";
      }
    } else {
    ...
    if (values.drvvt > ref.drvvt.high) {
      if (values.drvvt_ratio > ref.drvvt_ratio.positive) {
        report.findings.drvvt =
          "The dRVVT screen is prolonged with a positive confirm ratio. This finding is POSITIVE for a lupus anticoagulant.";
        isLACPositive = true;
      } else if (values.drvvt_ratio > ref.drvvt_ratio.indeterminate) {
        report.findings.drvvt =
          "The dRVVT screen is prolonged with an indeterminate confirm ratio. This result is equivocal and may represent a weak lupus anticoagulant or interference from other factors.";
        isLACIndeterminate = true;
      } else {
        report.findings.drvvt =
          "The dRVVT screen is prolonged but the confirm ratio is negative. This suggests a coagulation factor deficiency (common pathway) or a specific factor inhibitor rather than a lupus anticoagulant.";
      }
    } else {
    ...
    if (values.pt > ref.pt.high) {
      if (values.ptt_la > ref.ptt_la.high) {
        report.findings.other.push(
          "PT/INR is prolonged. When seen with a prolonged aPTT, this may indicate a common pathway factor deficiency (II, V, X), multiple factor deficiencies (e.g., liver disease, DIC), or certain anticoagulants."
        );
      } else {
        report.findings.other.push(
          "PT/INR is prolonged with normal aPTT. This pattern suggests factor VII deficiency, early warfarin effect, or liver dysfunction affecting vitamin K-dependent factors."
        );
      }
    }
    ...
    if (report.overall === "Negative" && report.caveats.length > 0) {
      report.overall = "Indeterminate";
      report.caveats.push(
        "The final result is indeterminate due to the presence of anticoagulants or other interfering factors, despite individual pathways being negative."
      );
    }
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    {errors.pt && <p className="text-sm text-red-600">{errors.pt}</p>}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    {errors.inr && <p className="text-sm text-red-600">{errors.inr}</p>}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    {errors.ptt_la && <p className="text-sm text-red-600">{errors.ptt_la}</p>}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    {errors.tt && <p className="text-sm text-red-600">{errors.tt}</p>}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    {errors.drvvt && <p className="text-sm text-red-600">{errors.drvvt}</p>}
                  </div>
                ...
                {report && (
                  <div className="mt-8 pt-8 border-t border-gray-200">
                    {/* Overall Result */}
                    <div
                      className={`p-4 rounded-lg border-2 ${getResultColors(report.overall)} mb-6`}
                    >
                        ...
                        <div className="bg-blue-50 rounded-lg p-4">
                          ...
                          <ul className="space-y-1">
                            {report.findings.other.map((finding, index) => (
                              <li
                                key={index}
                                className="text-gray-700 text-sm leading-relaxed flex items-start space-x-2"
                              >
                        ...
                        <div className="bg-yellow-50 border-l-4 border-yellow-400 rounded-lg p-4">
                          ...
                          <ul className="space-y-1">
                            {report.caveats.map((caveat, index) => (
                              <li
                                key={index}
                                className="text-yellow-800 text-sm leading-relaxed flex items-start space-x-2"
                              >
                ...
                <div className="mt-8 pt-6 border-t border-gray-200">
                  ...
                  <div className="text-sm text-gray-600 leading-relaxed flex items-start space-x-2">
                    ...
                    <span>
                      Devreese, K. M., de Groot, P. G., de Laat, B., Erkan, D., Favaloro, E. J.,
                      Mackie, I., et al. (2020). Guidance from the Scientific and Standardization
                      Committee for lupus anticoagulant/antiphospholipid antibodies of the
                      International Society on Thrombosis and Haemostasis. <em>J Thromb Haemost</em>
                      , 18(11), 2828-2839.
                    </span>
                  </div>
                </div>
