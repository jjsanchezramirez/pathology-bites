...
export default function CitationGeneratorPage() {
  ...
  const detectInputType = (input: string): 'url' | 'doi' | 'isbn' | 'unknown' => {
    ...
    if (input.includes('doi.org/') || input.includes('dx.doi.org/')) {
      return 'doi'
    }
    
    // ISBN patterns
    if (input.match(/^(?:ISBN(?:-1[03])?:?\s*)?(?=[-0-9\s]{17}$|[-0-9X\s]{13}$|[0-9X]{10}$)(?:97[89][-\s]?)?[0-9]{1,5}[-\s]?(?:[0-9]+[-\s]?){2}[0-9X]$/i)) {
      return 'isbn'
    }
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const metadata = await extractMetadata(input.trim(), inputType)
          ...
          <div className="flex items-center justify-between gap-8">
            ...
            <div className="flex-1 space-y-6 max-w-2xl">
              ...
              <p className="text-lg text-muted-foreground">
                Generate properly formatted citations for books, journal articles, and websites.
                Simply enter a URL, DOI, or ISBN and get APA, MLA, AMA, and Vancouver citations instantly.
              </p>
