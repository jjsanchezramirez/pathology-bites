...
export class ABPathPDFGenerator {
  ...
  private drawTable(
    ...
    rows: string[][],
    startY: number,
    options?: { columnStyles?: { [key: number]: { cellWidth: number } } }
  ) {
  ...
  private generateSummaryPage(
    ...
    if (
      ...
      if (filters.selectedType !== "all")
        this.addText(`Type: ${filters.selectedType.toUpperCase()}`);
      if (filters.selectedDesignation !== "all")
        this.addText(`Level: ${filters.selectedDesignation}`);
      this.currentY += 3;
    }
    ...
    const overallRows = [
      ...
      [
        "Advanced Resident (AR)",
        filters.stats.arCount.toString(),
        `${filters.stats.arPercentage}%`,
      ],
  ...
  private calculateSectionStats(section: PathologySection): {
    total: number;
    core: number;
    ar: number;
    fellow: number;
