...
export class ABPathPDFGenerator {
  ...
  private drawTable(headers: string[], rows: string[][], startY: number) {
    ...
    headers.forEach((header, i) => {
      this.doc.text(header, this.margin + (i * columnWidth) + 2, currentTableY + 2);
    });
    ...
    rows.forEach((row, rowIndex) => {
      ...
      row.forEach((cell, cellIndex) => {
        this.doc.text(cell, this.margin + (cellIndex * columnWidth) + 2, currentTableY + 2);
      });
      currentTableY += 4;
    });
  ...
  private generateContentPages(filteredData: PathologySection[]) {
    ...
    filteredData.forEach(section => {
      ...
      // Section header
      this.addColumnSubtitle(`${section.type.toUpperCase()} ${section.section}: ${section.title}`);
