...
export default function QuizTestPage() {
  ...
  const questions = Array.from({ length: totalQuestions }, (_, i) => ({
    id: i + 1,
    status: i < currentQuestion - 1 ? 'correct' : i === currentQuestion - 1 ? 'current' : 'unanswered'
  }))
                ...
                {questions.map((question) => (
                  <button
                    ...
                    className={cn(
                      ...
                      question.status === 'current' ? "bg-primary text-primary-foreground" : "bg-transparent hover:bg-muted"
                    )}
                  >
                    ...
                    <div className="flex-1">
                      <div className={cn(
                        ...
                        question.status === 'current' ? "text-primary-foreground" : "text-foreground"
                      )}>
                        Q{question.id}
                      </div>
                      <div className={cn(
                        "text-[12px] truncate",
                        question.status === 'current' ? "text-primary-foreground/80" : "text-muted-foreground"
                      )}>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-foreground leading-relaxed">
                        A 45-year-old woman presents with a firm, non-tender mass in the upper outer quadrant of her left breast.
                        Mammography shows a spiculated lesion. Which of the following is the most likely diagnosis?
                      </p>
                    </div>

                    <div className="space-y-2">
                      {['Fibroadenoma', 'Infiltrating ductal carcinoma', 'Phyllodes tumor', 'Fat necrosis'].map((option, i) => (
                        <button
                          key={i}
                          className={cn(
                    ...
                    <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-950/20 rounded-lg border border-blue-200 dark:border-blue-800">
                      ...
                      <p className="text-sm text-blue-800 dark:text-blue-200">
                        The presentation of a spiculated lesion on mammography in a middle-aged woman is highly suggestive
                        of infiltrating ductal carcinoma. This is the most common type of invasive breast cancer.
                      </p>
                    </div>
                  </div>
                </CardContent>
