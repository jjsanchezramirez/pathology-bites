...
export default function NewQuizPage() {
  ...
  // Load previous quiz titles for autogenerated naming
  useEffect(() => {
    const loadPreviousQuizTitles = async () => {
      try {
        setLoadingTitles(true)
        const response = await fetch('/api/quiz/sessions?limit=1000')
        if (response.ok) {
          const data = await response.json()
          const titles = data.data?.map((session: any) => session.title) || []
          setPreviousQuizTitles(titles)
        }
      } catch (error) {
        console.error('Error loading previous quiz titles:', error)
      } finally {
        setLoadingTitles(false)
      }
    }

    loadPreviousQuizTitles()
  }, [])
