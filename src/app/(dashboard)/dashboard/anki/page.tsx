...
export default function AnkiPage() {
  ...
  const formatTagName = useCallback(
    (tagName: string, parentCategory?: string): string => {
      ...
      if (
        parentCategory === "Microbiology" &&
        !MICROBIOLOGY_VALID_SUBCATEGORIES.includes(formatted)
      ) {
    ...
    [
      NAME_NORMALIZATIONS,
      PARENT_SPECIFIC_NORMALIZATIONS,
      CATEGORY_MERGES,
      MICROBIOLOGY_VALID_SUBCATEGORIES,
    ]
  );
  ...
  const currentCards = useMemo(() => {
    ...
    return isShuffled
      ? [...selectedCategory.cards].sort(() => Math.random() - 0.5)
