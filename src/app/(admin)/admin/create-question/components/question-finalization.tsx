...
export function QuestionFinalization({
  question,
  attachedImages,
  onQuestionCreated,
}: QuestionFinalizationProps) {
  ...
  const getAutoQuestionSetId = (aiModel: string, availableQuestionSets: QuestionSet[]): string => {
    ...
    for (const specificName of specificSetNames) {
      const exactMatch = availableQuestionSets.find(
        (set) => set.name.toLowerCase() === specificName.toLowerCase()
      );
      if (exactMatch) {
    ...
    const aiGeneratedSet = availableQuestionSets.find((set) => set.source_type === "ai_generated");
    if (aiGeneratedSet) {
      return aiGeneratedSet.id;
    }

    // Final fallback to any set with "AI" in the name
    const aiNamedSet = availableQuestionSets.find(
      (set) => set.name.toLowerCase().includes("ai") || set.name.toLowerCase().includes("generated")
    );
    if (aiNamedSet) {
      ...
      try {
        ...
        if (setsResponse.ok) {
          ...
          if (autoQuestionSetId) {
            ...
            const selectedSet = loadedQuestionSets.find(
              (set: QuestionSet) => set.id === autoQuestionSetId
            );
            console.log("Auto-selected question set:", selectedSet?.name, "ID:", autoQuestionSetId);
          } else {
            // Fallback to any AI-generated set
            const aiSet = loadedQuestionSets.find(
              (set: QuestionSet) => set.source_type === "ai_generated"
            );
            if (aiSet) {
              ...
              const aiNamedSet = loadedQuestionSets.find(
                (set: QuestionSet) =>
                  set.name.toLowerCase().includes("ai") ||
                  set.name.toLowerCase().includes("generated")
              );
              if (aiNamedSet) {
                setSelectedQuestionSet(aiNamedSet.id);
                console.log(
                  "Auto-selected AI-named question set:",
                  aiNamedSet.name,
                  "ID:",
                  aiNamedSet.id
                );
        ...
        if (question?.suggested_tags && question.suggested_tags.length > 0) {
          ...
          for (const suggestedTag of question.suggested_tags) {
            ...
            let existingTag = loadedTags.find(
              (tag: Tag) => tag.name.toLowerCase().trim() === suggestedTag.toLowerCase().trim()
            );
              ...
              try {
                ...
                if (response.ok) {
                  ...
                  const tagsResponse = await fetch(
                    "/api/admin/tags?page=0&pageSize=1000&search=" +
                      encodeURIComponent(suggestedTag.trim())
                  );
                  if (tagsResponse.ok) {
                    const tagsData = await tagsResponse.json();
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <Badge variant="outline">{question.question_options.length} options</Badge>
            <Badge variant="outline">
              Correct:{" "}
              {correctAnswer
                ? `Option ${question.question_options.indexOf(correctAnswer) + 1}`
                : "None"}
            </Badge>
            <Badge variant="outline">Images: {attachedImages.length} attached</Badge>
            <Badge variant="outline">Type: Multiple Choice</Badge>
            <Badge variant="outline">Status: {question.status}</Badge>
          </div>
        </CardContent>
      </Card>
                ...
                {categories.map((category) => {
                  ...
                  if (isParentCategory) {
                    ...
                    return (
                      <div
                        key={category.id}
                        className="px-2 py-1.5 text-sm font-semibold text-muted-foreground bg-muted/50 cursor-default"
                      >
                        {category.name}
                      </div>
                    );
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              <Check className="h-4 w-4 text-green-500" />
              <span className="text-green-700">Question structure valid</span>
            </div>
          </div>
        </CardContent>
      </Card>
