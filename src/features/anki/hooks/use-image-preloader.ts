...
export function useImagePreloader(
  ...
  const { enabled = true, preloadCount = 5, priority = "low" } = options;
  ...
  const getCardImageUrls = useCallback((card: AnkiCard): string[] => {
    const urls: string[] = [];
  ...
  const queueImagesForPreload = (urls: string[]) => {
    const newUrls = urls.filter(
      (url) => !preloadedImages.current.has(url) && !preloadQueue.current.includes(url)
    );
  ...
  useEffect(() => {
    ...
    const upcomingCards = cards.slice(currentIndex + 1, currentIndex + 1 + preloadCount);
...
export function useImageListPreloader(
  ...
  useEffect(() => {
    ...
    // Preload all images
    Promise.allSettled(imageUrls.map((url) => preloadImage(url))).then(() => {
