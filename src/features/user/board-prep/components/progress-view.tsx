...
export function ProgressView({ schedule, config, completedTasks, colorMap }: ProgressViewProps) {
  ...
  const byPhase: { name: string; completed: number; total: number; phaseIdx: number; minutes: number; completedMin: number }[] = [];
  ...
  const byResource: Record<string, { completed: number; total: number; minutes: number; completedMin: number }> = {};
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-border bg-card p-5 lg:p-6">
        <div className="flex items-end justify-between">
          ...
          <div className="text-right text-xs tabular-nums text-muted-foreground">
            <div>{completedCount} of {totalTasks} tasks</div>
            <div>{fmtHours(completedMinutes)} / {fmtHours(totalMinutes)}</div>
          </div>
        </div>
      ...
      {byPhase.length > 0 && (
        <div>
          <h3 className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Phases</h3>
              ...
              return (
                <div key={p.name} className="rounded-2xl border border-border bg-card p-4">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-xl font-bold tabular-nums" style={{ color: palette.accent }}>
                      {pct}%
                    </div>
                  </div>
      ...
      {Object.keys(byResource).length > 0 && (
        <div>
          <h3 className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Resources</h3>
          <div className="grid gap-3 sm:grid-cols-2">
            {Object.entries(byResource).map(([resName, stats]) => {
              const colors = colorMap[resName] || { bg: "#e5e5e5", text: "#000000" };
              const pct = stats.total > 0 ? Math.round((stats.completed / stats.total) * 100) : 0;
              return (
                <div key={resName} className="overflow-hidden rounded-2xl border border-border bg-card p-4">
                  <div className="flex items-center gap-3">
                    <div className="size-4 shrink-0 rounded-md" style={{ backgroundColor: colors.bg }} />
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between">
                        <span className="truncate text-sm font-medium text-foreground">{resName}</span>
                        <span className="ml-2 shrink-0 text-sm font-semibold tabular-nums text-foreground">{pct}%</span>
                      </div>
                      ...
                      <div className="mt-1.5 flex items-center justify-between text-[11px] tabular-nums text-muted-foreground">
                        ...
                        <span>{fmtHours(stats.completedMin)} / {fmtHours(stats.minutes)}</span>
                      </div>
                    </div>
                  </div>
                </div>
