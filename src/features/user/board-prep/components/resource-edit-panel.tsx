      ...
      <div className="flex items-center gap-2 border-b border-border px-4 py-3">
        ...
        {onDelete && !isNew && (
          <Button
            ...
            onClick={() => {
              if (confirm(`Remove ${form.name}?`)) onDelete();
            }}
          >
            Remove
          </Button>
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                if (v === "" || /^\d*\.?\d*$/.test(v))
                  setForm({ ...form, pace: parseFloat(v) || 0 });
              }}
            />
          </div>
        </div>
        ...
        <div>
          <div className="mb-2 flex items-center justify-between">
            <label className="text-xs text-muted-foreground">
              Subjects ({activeCount}/{subjects.length})
            </label>
          </div>

          <button
            onClick={() => onEditSubjects(form)}
          ...
          {subjects.length > 0 && (
            <div className="rounded-xl border border-border divide-y divide-border">
              {subjects.map((subj, idx) => (
                ...
                >
                  <div
                    className={`flex size-4 shrink-0 items-center justify-center rounded border ${subj.active !== false ? "border-primary bg-primary" : "border-muted-foreground/30"}`}
                  >
                    {subj.active !== false && (
