...
import { CompactAnswerOptions } from "../shared/compact-answer-options";
import { QuestionOptionFormData, QuestionImageFormData } from "@/shared/types/questions";
import { apiClient } from "@/shared/utils/api/api-client";
import { ImageData } from "@/shared/types/images";
import { TagData, CategoryData } from "@/shared/types/questions";
...
function MediaSection({ images, section, maxImages, onImagesChange }: MediaSectionProps) {
  ...
  const loadImages = React.useCallback(async () => {
    try {
      const { fetchImages } = await import("@/features/admin/images/services/images");
            ...
            <div key={uniqueKey} className="relative group aspect-square">
              <div className="w-full h-full bg-muted rounded border flex items-center justify-center">
                {imageInfo ? (
                  ...
                  <div className="text-xs text-muted-foreground text-center p-1">Loading...</div>
                )}
              </div>
              <Button
                type="button"
        ...
        <DialogPortal>
          ...
          <DialogContent className="!max-w-[1090px] !w-[1090px] max-h-[85vh] overflow-hidden border-0">
                ...
                {availableImages.length === 0 ? (
                  ...
                  <div className="grid grid-cols-5 gap-3">
                    {availableImages.map((image) => {
                      ...
                      const canSelect =
                        !isAlreadyAdded &&
                        (isSelected || selectedImageIds.length < maxImages - images.length);
            ...
            <div className="flex justify-end gap-2 pt-4">
              ...
              <Button onClick={handleSelectImages} disabled={selectedImageIds.length === 0}>
                Select {selectedImageIds.length > 0 ? `${selectedImageIds.length} ` : ""}Image
...
export function CreateQuestionDialog({ open, onOpenChange, onSave }: CreateQuestionDialogProps) {
  ...
  useEffect(() => {
    const hasChanges =
      answerOptions.some(
        (option) => option.text.trim() !== "" || option.explanation?.trim() !== ""
      ) ||
  ...
  const validateAnswerOptions = (): Record<string, string> => {
    ...
    answerOptions.forEach((option, index) => {
      console.log(`🔍 Checking option ${index}:`, {
        text: option.text,
      ...
      <DialogPortal>
        ...
        <DialogContent className="w-full max-w-[min(85vw,1400px)] sm:max-w-[min(85vw,1400px)] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Create a new question for your question bank. You can add images and associate it with
              a question set.
            </DialogDescription>
          </DialogHeader>
                    ...
                    <div className="space-y-6">
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <span className="text-xs text-muted-foreground">
                            {questionImages.filter((img) => img.question_section === "stem").length}
                            /3
                          </span>
                        </div>
                        <MediaSection
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <span className="text-xs text-muted-foreground">
                            {
                              questionImages.filter((img) => img.question_section === "explanation")
                                .length
                            }
                            /1
                          </span>
                        ...
                        <MediaSection
                          images={questionImages.filter(
                            (img) => img.question_section === "explanation"
                          )}
                          section="explanation"
                          maxImages={1}
                          onImagesChange={(newImages) => {
                            const stemImages = questionImages.filter(
                              (img) => img.question_section === "stem"
                            );
        ...
        <DialogPortal>
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                You have unsaved changes. Are you sure you want to close this dialog? All changes
                will be lost.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
