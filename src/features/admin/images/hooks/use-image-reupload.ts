...
export function useImageReupload({
  ...
  const reuploadImage = useCallback(
    async (imageId: string, file: File, updateMetadata: boolean = false) => {
      if (!file) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            result.error || result.message || `HTTP ${response.status}: Failed to replace image`
          );
        }
        ...
        if (metadata) {
          const sizeChange =
            metadata.newSize > metadata.oldSize
              ? "larger"
              : metadata.newSize < metadata.oldSize
                ? "smaller"
                : "same size";
