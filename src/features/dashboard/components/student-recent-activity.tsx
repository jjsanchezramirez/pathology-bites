          ...
          {activities && activities.length > 0 ? (
            activities.map((activity) => {
              const activityContent = (
                <div
                  className={`flex items-center gap-3 rounded-md border p-3 transition-all duration-200 ${
                    activity.navigationUrl
                      ? "hover:bg-muted/30 hover:border-muted-foreground/30 cursor-pointer"
                      : ""
                  }`}
                >
                  ...
                  <div className="flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center bg-muted">
                    {activity.type === "quiz_completed" && (
                      <Award className="h-3.5 w-3.5 text-[hsl(var(--chart-1))]" />
                    )}
                    {activity.type === "study_streak" && (
                      <TrendingUp className="h-3.5 w-3.5 text-muted-foreground" />
                    )}
                    {activity.type === "quiz_started" && (
                      <Play className="h-3.5 w-3.5 text-[hsl(var(--chart-2))]" />
                    )}
                    {activity.type === "achievement_unlocked" && (
                      <Trophy className="h-3.5 w-3.5 text-[hsl(var(--chart-4))]" />
                    )}
