...
export function QuestionSnapshotPreview({
  ...
  const incorrectOptions = Array.isArray(question.question_options)
    ? question.question_options.filter(
        (option: unknown) => !option.is_correct && option.explanation
      )
    : [];
    ...
    <div className={`space-y-4 ${isComparison ? "text-sm" : ""}`}>
              ...
              return (
                ...
                >
                  ...
                  {si.image.description && (
                    <div
                      className={`p-2 ${isComparison ? "text-xs" : "text-sm"} text-muted-foreground bg-background/50`}
                    >
                      {si.image.description}
                    </div>
                  )}
      ...
      <div className="space-y-2">
        ...
        {Array.isArray(question.question_options) && question.question_options.length > 0 ? (
          ...
          <div
            className={`p-3 text-center text-muted-foreground ${isComparison ? "text-xs" : "text-sm"}`}
          >
      ...
      {(question.teaching_point ||
        ...
        <div className="p-3 rounded-lg bg-muted/50 space-y-4">
          ...
          {question.teaching_point && (
            <div>
              <h5 className={`font-medium ${isComparison ? "text-xs" : "text-sm"} uppercase mb-1`}>
                Teaching Point
              </h5>
          ...
          {Array.isArray(incorrectOptions) && incorrectOptions.length > 0 && (
            <div>
              <h5 className={`font-medium ${isComparison ? "text-xs" : "text-sm"} uppercase mb-2`}>
                Incorrect Option Explanations
              </h5>
              <div className="space-y-2">
                {incorrectOptions.map((option: unknown, index: number) => {
                  const originalIndex =
                    question.question_options?.findIndex((opt: unknown) => opt.id === option.id) ||
                    0;
                  const optionLabel = optionLabels[originalIndex] || (originalIndex + 1).toString();

                  return (
                    <div
                      key={option.id || `incorrect-option-${index}`}
          ...
          {explanationImages.length > 0 && (
            <div>
              <h5
                className={`font-medium ${isComparison ? "text-xs" : "text-sm"} uppercase mb-${isComparison ? "1" : "2"}`}
              >
                Reference Images
              </h5>
              <div className={`space-y-${isComparison ? "2" : "3"}`}>
      ...
      {/* Metadata */}
      <div
        className={`flex items-center gap-4 pt-2 border-t ${isComparison ? "text-xs" : "text-sm"} text-muted-foreground`}
      >
        ...
        <span>
          Status:{" "}
          {question.status?.charAt(0).toUpperCase() + question.status?.slice(1) || "Published"}
        </span>
