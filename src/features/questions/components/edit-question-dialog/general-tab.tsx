...
export function GeneralTab({ form, question, onUnsavedChanges, onTagsChange }: GeneralTabProps) {
    ...
    const loadData = async () => {
      try {
        ...
        const [tagsResult, categoriesResult] = await Promise.all([
          // Get 10 most recently created tags
          supabase.from('tags').select('*').order('created_at', { ascending: false }).limit(10),
          supabase.from('categories').select('*').order('name')
        ]);

        if (tagsResult.data) {
          // Merge with existing question tags to ensure they're available
          const existingTags = question?.tags || [];
          const allTags = [...tagsResult.data];

          // Add any question tags that aren't in the recent tags
  ...
  // Separate effect to handle onTagsChange callback
  useEffect(() => {
    if (question) {
      const tagIds = question.tags?.map(tag => tag.id) || [];
      onTagsChange?.(tagIds);
    }
  }, [question, onTagsChange]);
