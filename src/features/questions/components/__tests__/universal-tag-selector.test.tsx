...
import { UniversalTagSelector } from '../universal-tag-selector';
import { useUniversalTags } from '../../hooks/use-universal-tags';

// Mock the hook
jest.mock('../../hooks/use-universal-tags');
const mockUseUniversalTags = useUniversalTags as jest.MockedFunction<typeof useUniversalTags>;
...
describe('UniversalTagSelector', () => {
  ...
  describe('Basic Rendering', () => {
    it('renders with default props', () => {
      ...
      expect(screen.getByPlaceholderText('Search tags or type to create new...')).toBeInTheDocument();
    });
  ...
  describe('Tag Selection', () => {
    ...
    it('allows removing selected tags', async () => {
      ...
      const removeButton = screen.getByRole('button', { name: /Remove Pathology tag/i });
    ...
    it('shows dropdown when input is focused', async () => {
      ...
      const input = screen.getByPlaceholderText('Search tags or type to create new...');
  ...
  describe('Tag Creation', () => {
    ...
    it('creates new tag when clicking create option', async () => {
      const user = userEvent.setup();
      const newTag = { id: '4', name: 'NewTag', created_at: '2024-01-04T00:00:00Z' };
      mockHookReturn.searchTags.mockResolvedValue([]);
  ...
  describe('Keyboard Navigation', () => {
    ...
    it('closes dropdown with Escape key', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByRole('button', { name: 'Pathology' })).not.toBeInTheDocument();
      });
    });
  });
  ...
  describe('Single Selection Mode', () => {
    ...
    it('hides input when tag is selected in single mode', () => {
      ...
      expect(screen.queryByPlaceholderText('Search tags or type to create new...')).not.toBeInTheDocument();
      expect(screen.getByText('Pathology')).toBeInTheDocument();
    });
  });
