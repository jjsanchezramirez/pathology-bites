...
export function SecurityMonitor({ showLowRisk = false, className }: SecurityMonitorProps) {
  ...
  const { detectSuspiciousActivity, getSecurityEvents } = useSessionSecurity()
  const [suspicious, setSuspicious] = useState(false)
  const [suspiciousReasons, setSuspiciousReasons] = useState<string[]>([])

  useEffect(() => {
    if (isAuthenticated) {
      const { suspicious: isSuspicious, reasons } = detectSuspiciousActivity()
      setSuspicious(isSuspicious)
      setSuspiciousReasons(reasons)
    }
  }, [isAuthenticated, detectSuspiciousActivity])
...
export function SecurityDebugPanel() {
  ...
  const { getSecurityEvents, detectSuspiciousActivity } = useSessionSecurity()
  ...
  useEffect(() => {
    if (isAuthenticated) {
      ...
      const { suspicious: isSuspicious } = detectSuspiciousActivity()
      setSuspicious(isSuspicious)
    }
  }, [isAuthenticated, getSecurityEvents, detectSuspiciousActivity])
