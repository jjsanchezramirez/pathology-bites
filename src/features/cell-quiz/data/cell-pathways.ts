...
export function findCellPathway(cellType: string): CellPathway | null {
  return CELL_PATHWAYS.find((pathway) => pathway.cells.includes(cellType)) || null;
}
...
export function generateBiologicalOptions(correctCellType: string): string[] {
  ...
  if (relationship.closelyRelated.length > 0) {
    const closeOption =
      relationship.closelyRelated[Math.floor(Math.random() * relationship.closelyRelated.length)];
  ...
  if (relationship.unrelated.length > 0) {
    const unrelatedOption =
      relationship.unrelated[Math.floor(Math.random() * relationship.unrelated.length)];
