...
async function compressImage(
  ...
  if (ext === ".gif") {
    const compressed = await sharp(fileBuffer, { animated: true }).gif().toBuffer();
    return {
      buffer: compressed,
      originalSize,
...
export async function POST() {
  try {
    ...
    console.log(
      `Starting Anki media upload with compression from ${mediaDir} to ${targetBucket}/anki/`
    );
    console.log(
      `Compression settings: max ${COMPRESSION_CONFIG.maxWidth}x${COMPRESSION_CONFIG.maxHeight}, JPEG quality ${COMPRESSION_CONFIG.jpeg.quality}, PNG quality ${COMPRESSION_CONFIG.png.quality}`
    );
    ...
    for (let i = 0; i < mediaFiles.length; i++) {
      ...
      try {
        ...
        const {
          ...
          compressedSize,
        } = await compressImage(fileBuffer, filename, ext);
